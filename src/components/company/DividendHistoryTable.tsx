    ...
    <div>
      <div className="overflow-hidden rounded-xl border border-border/70">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-secondary/50 text-left font-mono text-[0.58rem] uppercase tracking-[0.08em] text-muted-foreground">
              <th className="px-3 py-2 font-medium">{strings.company.exDate}</th>
              <th className="px-3 py-2 text-right font-medium">{strings.company.paid}</th>
              <th className="px-3 py-2 text-right font-medium">{strings.company.adjusted}</th>
              <th className="px-3 py-2 text-right font-medium">{strings.company.cumulative}</th>
          ...
          <tbody>
            {dividends.map((d, i) => (
              <tr key={`${d.exDate}-${i}`} className="border-b border-border/50 last:border-0">
                ...
                <td className="px-3 py-2 text-right font-mono tnum">{formatSar(d.cumulative)}</td>
                <td className="px-2 py-2 text-right">{!d.verified && <UnverifiedBadge />}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="mt-3 grid grid-cols-3 gap-2">
        <Tot label={strings.company.totalDividends} value={formatSar(total)} />
        <Tot label={strings.company.payments} value={String(count)} />
        <Tot
          label={strings.company.dividendYield}
